import { DataTypes } from './db/data-types';
import { Database } from './db/database';
import { Composer } from './composer';
import { ModelArray } from './model-array';
import { Utilities } from './utilities';
import * as async from 'async';
import { RelationShipPath } from './relationship-graph';
/**
 * 
 */
export class Model {
    ...
    public _validations = {};
    public _validationsList = [];
    public _calculations = {};
    public _calculationsList = [];
    public _verificationsList = [];
    public _hides = {};
    public formatters = {};
    public data = null;
    public db = null;
    ...
    public inflect = require('i')();
    public deepEqual = require('deep-equal');
    ...
    */
    constructor(modelData: any, fromStorage?: boolean) {
        modelData = modelData || {};
        this.__initialize__();
        this.__load__(modelData, fromStorage);
        this.Relationships = new RelationShipPath();
    }
    ...
    * @param {function({Error} err, {Nodal.Model} model)} callback The callback to execute upon completion
    */
    static findBy(field, value, callback, joinedBy) {

        let db = this.prototype.db;
        let query = {};
        query[field] = value;
        let joinField = joinedBy || false;

        const objFindBy = new Composer(this);
        if (joinField) {
            objFindBy.join(joinField);
        }
        objFindBy.where(query)
        objFindBy.end((err, models) => {
            if (!err && !models.length) {
